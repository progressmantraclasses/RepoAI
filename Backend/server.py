...
from serpapi.serpapi_client import GoogleSearch
...
def load_model(path):
    ...
    absolute_path = os.path.join(os.getcwd(), path) # Get the absolute path

    try:
        with open(absolute_path, 'rb') as file: # Open the file using absolute path
            return pickle.load(file)
    except Exception as e:
        raise RuntimeError(f"Error loading model: {str(e)}")

# Ensure the model.pkl and station.pkl files are in the same directory as your script
